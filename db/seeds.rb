...
if Rails.env.development?
  ...
  csv.each do |row|
    ...
    Status.create(account_id: account.id, text: row['Tweet'], created_at: Time.now.utc + counter * 10, updated_at: Time.now.utc + counter * 10, local: true, sentiment_score: row['Labels'].to_f )
    counter = counter + 1
  end
